...
const diagnostics = new Diagnostics({
  ...
  alert: (info) => {
    ...
    console.log(`   Memory: ${info.memory.formatted.current} (threshold: ${info.memory.formatted.threshold})`);
  },
...
function complete() {
  ...
  if (report.analysis.target) {
    ...
    console.log('├─ Time Over Target:', report.analysis.timeOverTargetHuman, `(${overTargetPercent}%)`);
    console.log('└─ Time Under Target:', report.analysis.timeUnderTargetHuman, `(${underTargetPercent}%)`);
  }
